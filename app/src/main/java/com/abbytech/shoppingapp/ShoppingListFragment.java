...
public class ShoppingListFragment extends Fragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        fab.setOnClickListener(v -> {
            ListItem listItem = insertNewItem();
    ...
    private ListItem insertNewItem() {
        Item item = ShoppingApp.getInstance().getItemRepo().getAllItems().toBlocking().first().get(0);
        ListItem listItem = new ListItem(item, shoppingList);
        ShoppingListRepo.getInstance().saveShoppingItem(listItem);
        return listItem;
    }

    public void loadShoppingList(int id) {
        Observable<ShoppingList> shoppingListObservable = ShoppingListRepo.getInstance()
                .getShoppingList(id);
